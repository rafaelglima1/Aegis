...
C7-02: SandboxBroker receives configured capital as initial balance.
"""
...
def create_broker(settings: Settings | None = None, initial_balance: Decimal | None = None) -> BrokerAdapter:
    ...
    C7-02: initial_balance syncs SandboxBroker with Portfolio capital.
    """
    if settings is None:
    ...
    if settings.trading_environment.value == "SANDBOX":
        balance = initial_balance if initial_balance is not None else settings.initial_capital
        return SandboxBroker(initial_balance=balance)
